...
## Session 68: compact long-stitch: auto-expand inline params panel on first image add

**Date**: 2026-05-27
**Task**: compact long-stitch: auto-expand inline params panel on first image add
**Branch**: `main`

### Summary

Wired StitchEditorController's existing ref.listen to flip stitchControlsInlineVisibleProvider on empty↔non-empty image-list edges (compact viewport only; medium/expanded/large untouched). Mid-session add/remove/reorder preserve the user's explicit [⚙ 参数] chip choice via a stateWasEmpty snapshot shared with the subtitle-reset rule. Added 8 controller-level tests covering R3 (auto-expand) / R4 (auto-collapse) / R5 (no clobber) plus first-fire guard and clear→re-import re-expand. Spec updates across three files: component-guidelines.md (compact editor 'auto-expand on image-list edges' UX convention), state-management.md (testing-nuance pointer on the existing ref.listen pattern), quality-guidelines.md (new 'Poke the consumer to fire ref.listen side-effects on lazy Provider.family derivations' pattern with readVisible helper, captured from a 6/8 test-failure debug session). Side commit (436286f): orientation card visual redesign — single distribute glyph replaced with a 1dp-framed box plus dual chevron arrows.
