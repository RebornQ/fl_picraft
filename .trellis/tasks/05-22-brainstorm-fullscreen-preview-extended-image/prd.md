...
* **R3-exception** (用户须接受的细微 UX 降级 / 实现差异):
  * **(1) drag-to-dismiss 实现路径** (**2026-05-23 修订**: 调用方保持
    `showDialog`，详见 Post-ST2 Revision 节): 用 `ExtendedImageSlidePage`
    (StatefulWidget) 包裹画廊；**外层保留** `Dialog.fullscreen(backgroundColor:
    Colors.transparent)`，调用方继续用 `showDialog<void>(barrierDismissible: false,
    builder: ...)`。`ExtendedImage(..., enableSlideOutPage: true)` 内建感知外层
    SlidePage 状态。**阈值定制**通过 `slideEndHandler(ScaleEndDetails)` 回调，
    命中 100dp 或 800dp/s fling 时返回 `true` 触发 pop。**背景 opacity**通过
    `slidePageBackgroundHandler(color, pageSize, offset)` 返回
    `Colors.black.withValues(alpha: 1.0 - clamp(|offset.dy|/100, 0, 1) * 0.6)`
    硬编码 1.0→0.4 曲线。
    ~~~~调用方把 `PreviewFullScreenDialog` 从 `showDialog<void>(Dialog.fullscreen(...))`
    迁移到 `Navigator.of(context).push(PageRouteBuilder<void>(opaque: false,
    barrierColor: ..., pageBuilder: ...))`（透明 PageRoute）~~~~（原文已废弃，
    ST2 阶段验证 `showDialog` 体验更丝滑，回滚此项；详见修订节）。
...
* **R6** (**2026-05-23 修订 — 见 Post-ST2 Revision 节**): 调用方
  `preview_thumbnail.dart::_openFullScreen` **保留 `showDialog<void>`**，
  **不**迁移到 `Navigator.push(PageRouteBuilder(opaque: false))`。
  ST2 完成后实际手测发现 `showDialog` 的打开/关闭过渡比透明 PageRoute 更丝滑
  （PageRouteBuilder 切换瞬间有可感知的视觉抖动 — 用户原话"showDialog 交互更
  丝滑"），故反向。对 `PreviewThumbnail` 调用方（grid / stitch）零修改不变。
  ~~原计划：调用方 `preview_thumbnail.dart::_openFullScreen` 从 `showDialog<void>`
  迁移到 `Navigator.of(context).push(PageRouteBuilder<void>(opaque: false, ...))`~~。
...
## Post-ST2 Revision (2026-05-23)

> **触发**: ST2 (`05-22-migrate-fullscreen-dialog`) 实施 + 人工测试后，笨蛋
> 反馈"showDialog 交互更丝滑，PageRouteBuilder 切换有可感知的视觉过渡微抖动"。

### Decision (reverse-decision against R6 / R3-exception (1))

**保留** `preview_thumbnail.dart::_openFullScreen` 的 `showDialog<void>(...)` 调用，
**不**迁移到 `Navigator.of(context).push(PageRouteBuilder<void>(opaque: false, ...))`。

`PreviewFullScreenDialog` 自身仍然走完整的 `extended_image` 三件套
（`ExtendedImageSlidePage + ExtendedImageGesturePageView.builder + ExtendedImage.memory(
mode: gesture, inPageView: true, enableSlideOutPage: true)`）—— 三件套的 drag-to-dismiss
+ 缩放协调能力完全不受 outer route 类型影响。仅**最外层**走 `Dialog.fullscreen(
backgroundColor: Colors.transparent)` + `showDialog`，让 dialog 打开/关闭过渡
保持 Material 默认动画（fade + scale），符合用户对 "Dialog" 视觉契约的预期。

### Why showDialog won over transparent PageRoute

| 维度 | `showDialog<void>(Dialog.fullscreen(transparent))` | `Navigator.push(PageRouteBuilder(opaque: false))` |
|---|---|---|
| 打开动画 | Material Dialog 默认 fade + 轻 scale，原生质感 | 由 caller 在 `transitionsBuilder` 实现，**默认**是直切 |
| 关闭动画 | 同上，关闭流畅 | 透明路由切换可见父屏，**视觉抖动**（pixel reflow） |
| Barrier 管理 | `barrierDismissible: false` 明确 + `Dialog` Material 自带 | 需手动控制 `barrierColor` + 透明路由的 dismiss 行为 |
| 调用方代码改动 | 0 行 | 5-10 行（route builder + transitions builder） |
| ST2 三件套兼容 | ✅（Dialog.fullscreen 外壳 + Slide page 内壳，已 ST2 验证）| ✅ 但需重测全链 |

ST2 已经在 dialog widget 内部把所有"自实现 ↔ extended_image"的迁移完成；
PageRouteBuilder 的迁移**没有**带来额外的功能价值，只有理论上的"更纯净的
透明路由"，但实测的视觉抖动让该选择变成净负。

### 影响范围（受本修订影响的 PRD 章节）

* **R6** — 重写为"保留 showDialog"（已修订）
* **R3-exception (1)** — `Dialog.fullscreen` 外壳保留改为最终方案，不再是过渡
  期 hack（已修订）
* **Implementation Plan ST2** — 原文"同步更新 `PreviewThumbnail._openFullScreen`
  从 `showDialog` 到 `Navigator.push(PageRouteBuilder(opaque: false, ...))`"
  **取消**（ST2 实际未执行该步，留在 PRD 中是历史规划）
* **Implementation Plan ST3** — 范围**收窄**为仅图片显示替换
  (`Image.memory` → `ExtendedImage.memory(mode: ExtendedImageMode.none)`)，
  **不动** `_openFullScreen` 方法

### 影响范围（不受本修订影响）

* **R1-R5** 全部生效，无修订
* **R3** 全部生效（chrome auto-hide / 三件套 / 缩放参数 / `_ImmersiveScrollBehavior` /
  letterbox 焦点 fallback 删除等均不变）
* **R3-exception (2)(3)(4)(5)** 全部生效（spring-back linear 降级 / M-α 删除 /
  `_currentZoomed` 删除 / 自定义 ScrollPhysics 删除）
* **ST1** 已 archived，不动
* **ST2** 已 archived，不动 —— ST2 commit `c132988` 中 `Dialog.fullscreen` 外壳
  实际已保留（前瞻性安全决策），所以无需 ST2 返工
* **ST4** 范围不变（重写测试 + 双 ADR），但 ADR-0002 文本写作时**需反映本修订**
  （记录"原计划迁移调用方，ST2 测试后反转决策保留 showDialog"作为 ADR-0002
  Consequences 段的一条 sub-bullet）

### Lessons for institutional memory

* 即使在 brainstorm 阶段做了详细方案选择（Approach A/B/C + ADR-lite），**实施后
  的实际手测仍可能反向某条具体决策**。这是健康的 PRD 演进，不算计划失误。
* "理论上更纯净的方案" (透明 PageRoute) 不一定胜过 "实际更顺手的方案"
  (`showDialog` Material 默认动画)。视觉过渡这种"细节维度"很难在 brainstorm
  阶段预判，**必须留出 ST 实施后的小范围回归窗口**让方案在真实交互下定锚。
* 此次反向并未推翻 Decision (ADR-lite) 主决策（Approach A: 三件套）—— 只是
  反向了 sub-decision（调用方路由类型）。**主决策与 sub-decision 分层管理**
  让反向决策的成本可控。
...
* **ST2**: `migrate-preview-full-screen-dialog` — 重写
  ...
  PageRouteBuilder(opaque: false, ...))`~~（**2026-05-23 修订取消** — 见 Post-ST2
  Revision 节；ST2 实际未执行此项，且 `Dialog.fullscreen` 外壳保留作为最终方案）。
* **ST3**: `migrate-preview-thumbnail` — 重写
  ...
  **范围（2026-05-23 修订）**: **仅**图片显示替换 (`Image.memory` →
  `ExtendedImage.memory(mode: ExtendedImageMode.none)`)，**不动** `_openFullScreen`
  方法（继续使用 `showDialog<void>(...)`，详见 Post-ST2 Revision 节）。
